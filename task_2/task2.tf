...
resource "aws_key_pair" "instance1key" {
  key_name = "task2-key"
...
resource "aws_instance" "myinstance1" {
  ...
  key_name      = aws_key_pair.instance1key.key_name
...
output "outavailabilityzone"{
  value=aws_instance.myinstance1.subnet_id
}
...
resource "aws_efs_mount_target" "mount_tar" {
  ...
  subnet_id      = aws_instance.myinstance1.subnet_id
}
...
resource "null_resource" "remote1" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo git clone https://github.com/Adamaya/hybrid-multi-cloud-tasks.git /var/www/html/",
    ]
  }
}
...
resource "aws_cloudfront_distribution" "s3_distribution" {
  ...
  provisioner "remote-exec" {
    inline = [
      "sudo su << EOF",
      "echo \"<img src='http://${self.domain_name}/${aws_s3_bucket_object.buck_obj.key}' height='500px' width='500px'>\" >> /var/www/html/index.php",
      "EOF",
    ]
  }
}